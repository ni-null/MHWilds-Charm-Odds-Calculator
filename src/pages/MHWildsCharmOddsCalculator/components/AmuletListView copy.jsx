...
export default function AmuletListView(props) {
  ...
  // Compute totals from the actual rendered items under the list (visibleAmulets).
  // If an amulet has expanded combo entries (from props.expandedMatches), use those
  // per-combo probabilities (probNoSlot/probWithSlot) because the UI renders one
  // list item per combo in that case. This keeps totals consistent with what's
  // visually rendered and avoids out-of-sync sums.
  const expandedMatches = Array.isArray(props.expandedMatches) ? props.expandedMatches : []
  visibleAmulets.forEach((amulet) => {
    const amuletExpanded = expandedMatches.filter((m) => m.amulet === amulet)
    if (amuletExpanded.length > 0) {
      // Sum each combo's probability (convert to percent to match rawPercent units)
      amuletExpanded.forEach((m) => {
        const noSlotPct = (m.probNoSlot || 0) * 100
        const withSlotPct = (m.probWithSlot || 0) * 100

        if (!rarityProbabilities[amulet.Rarity]) {
          rarityProbabilities[amulet.Rarity] = { noSlot: 0, withSlot: 0 }
        }
        rarityProbabilities[amulet.Rarity].noSlot += noSlotPct
        rarityProbabilities[amulet.Rarity].withSlot += withSlotPct
        totalNoSlot += noSlotPct
        totalWithSlot += withSlotPct
      })
    } else {
      ...
      {visibleAmulets.length > 0 ? (
        <ul className='divide-y divide-gray-200'>
          {visibleAmulets.map((amulet, index) => {
            ...
            const selectedSkillsFilteredForRender = [].concat(...selectedSkills.map((a) => (Array.isArray(a) ? a : a ? [a] : []))).filter(Boolean)
            ...
            // remove exact-duplicate skill entries (same skill name and level), keep first occurrence
            const uniqueSortedSelectedSkills = sortedSelectedSkills.filter((v, i, arr) => arr.indexOf(v) === i)
                ...
                <li className='flex flex-col items-start justify-between gap-4 px-2 py-4 border-b md:flex-row md:items-center md:gap-6'>
                  ...
                  <div className='flex-1'>
                    ...
                    <div className='mb-1 text-base'>
                      ...
                      {uniqueSortedSelectedSkills.map((skillKey, skillIndex) => {
                  ...
                  <li className='px-2 py-3 border-b bg-gray-50'>
                    <div className='p-3 text-sm bg-white border rounded'>
                      {(() => {
                        ...
                        const selectedSkillsFiltered = [].concat(...selectedSkills.map((a) => (Array.isArray(a) ? a : a ? [a] : []))).filter(Boolean)
                        ...
                        // build combinations from per-slot arrays
                        const perSlotArrays = selectedSkills.map((a) => (Array.isArray(a) ? a : a ? [a] : []))
                        // helper: cartesian product
                        const buildCombinations = (arrays) => {
                          const res = []
                          const dfs = (idx, acc) => {
                        ...
                        const combos = buildCombinations(perSlotArrays)
                        ...
                        // build skill->groups map for groups present in this amulet
                            ...
                            if (assignedSlot !== -1) {
                              ...
                              if (SkillGroupsData.SkillGroups[groupKey]) {
                                ...
                                allSelectedBaseNames.forEach((baseName) => {
                                  const sameBaseNameSkills = SkillGroupsData.SkillGroups[groupKey].data.filter(
                                    (skill) => skill.SkillName === baseName
                                  )
                        ...
                        // build a sample detail (first combo) for debug display clarity
                        ...
                        if (combos && combos[0]) {
                          ...
                          sampleUsedSkillCounts = []
                          sampleSkillCombProb = 1
                          for (const skillKey of combo) {
                            const skillGroups = skillToGroupMapLocal[skillKey] || []
                            let assignedSlot = -1
                            for (let slotIndex = 0; slotIndex < amuletGroups.length; slotIndex++) {
                            ...
                            if (assignedSlot !== -1) {
                              ...
                              if (SkillGroupsData.SkillGroups[groupKey]) {
                                const allSelectedBaseNames = new Set()
                                Object.values(selectedSkillBaseNamesInGroupSample).forEach((baseNameSet) => {
                                  baseNameSet.forEach((baseName) => allSelectedBaseNames.add(baseName))
                                })
                                allSelectedBaseNames.forEach((baseName) => {
                                  const sameBaseNameSkills = SkillGroupsData.SkillGroups[groupKey].data.filter(
                        ...
                        return (
                          <>
                            ...
                            <div className='mb-1 text-base'>
                              {t("probability.debug.skillCombProb")} : {sampleUsedSkillCounts.map((count) => `1/${count}`).join(" × ")} ={" "}
                              {(() => {
                                const prob = sampleSkillCombProb
                                if (prob >= 0.001) return prob.toFixed(8)
                                else if (prob >= 0.0001) return prob.toFixed(10)
                                else return prob.toFixed(12).replace(/\.?0+$/, "")
                              })()}
